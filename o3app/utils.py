...
def require_xlsx_file(path: Path, field_label: str) -> None:
    if not path.is_file():
        raise ValueError(f"Please select a valid {field_label}.")
    if not path.name.lower().endswith(".xlsx") or not zipfile.is_zipfile(path):
        raise ValueError(
            f"The {field_label} must be an Excel .xlsx workbook, but '{path.name}' "
            "is not one. The value-mapping CSV belongs only in the 'Value Mapping CSV' "
            "field; pick the .xlsx mapping workbook instead."
        )


def require_value_mapping_csv(path: Path) -> None:
    if not path.name.lower().endswith(".csv"):
        raise ValueError(
            f"The 'Value Mapping CSV' field expects a .csv file, but '{path.name}' "
            "is not a CSV. Leave the field empty if you have no value mapping file."
        )
